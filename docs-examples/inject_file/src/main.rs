...
fn injected(source_text: &str, get_path: fn(&str) -> String) -> Result<String, InjectError> {
    ...
    let re_wo_marker = Regex::new(r"<load path='(.*)'.*>\n?").unwrap();
    ...
    let read_or_download_to_string = |path: &str| {
        read_to_string(path).ok().or_else(|| {
            Some(reqwest::blocking::get(path).ok()?.error_for_status().ok()?.text().ok()?)
        })
    };
    ...
    let result = re.replace_all(source_text, |caps: &Captures| {
        ...
        let Some(to_inject) = read_or_download_to_string(&path) else {
